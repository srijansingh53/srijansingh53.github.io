...
(self => {
  ...
  const run = (env, scriptContent, scriptUrl) => {
    ...
    scriptContent =
      `with(this){${scriptContent.replace(/\bthis\b/g, (match, offset, originalStr) => (offset > 0 && '$' !== originalStr[offset - 1] ? '(thi$(this)?window:this)' : match)).replace(/\/\/# so/g, '//Xso')}\n;function thi$(t){return t===this}};${(
            ...
            let nav = {
              sendBeacon: (url, body) => {
                if (webWorkerCtx.$config$.logSendBeaconRequests) {
                  try {
                    logWorker(
                      `sendBeacon: ${resolveUrl(env, url, null)}${body ? ', data: ' + JSON.stringify(body) : ''}`,
                    );
                  } catch (e) {
                    console.error(e);
                  }
