((this.workbox = this.workbox || {}),
  (this.workbox.expiration = (function (t, e, s, i, a, n) {
    ...
    class o {
      constructor(t) {
        ((this.t = t), (this.s = new e.DBWrapper(h, 1, { onupgradeneeded: t => this.i(t) })));
      }
      i(t) {
      ...
      async setTimestamp(t, e) {
        ((t = r(t)),
          await this.s.put(c, { url: t, timestamp: e, cacheName: this.t, id: this.h(t) }));
      }
      ...
      async expireEntries(t, e) {
        const s = await this.s.transaction(c, 'readwrite', (s, i) => {
            const a = s.objectStore(c),
              n = [];
            let h = 0;
            a.index('timestamp').openCursor(null, 'prev').onsuccess = ({ target: s }) => {
              const a = s.result;
              if (a) {
                const s = a.value;
                (s.cacheName === this.t &&
                  ((t && s.timestamp < t) || (e && h >= e) ? n.push(a.value) : h++),
                  a.continue());
              } else i(n);
            };
          }),
    ...
    return (
      ...
      (t.Plugin = class {
        ...
        cachedResponseWillBeUsed({ event: t, request: e, cacheName: s, cachedResponse: i }) {
          if (!i) return null;
          let a = this.N(i);
          const n = this.k(s);
          n.expireEntries();
