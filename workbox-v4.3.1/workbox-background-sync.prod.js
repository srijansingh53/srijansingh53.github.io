((this.workbox = this.workbox || {}),
  (this.workbox.backgroundSync = (function (t, e, s) {
    ...
    class c {
      constructor(t) {
        ((this.t = t), (this.s = new s.DBWrapper(n, i, { onupgradeneeded: this.i })));
      }
      async pushEntry(t) {
      ...
      async getAll() {
        return await this.s.getAllMatching(a, { index: r, query: IDBKeyRange.only(this.t) });
      }
      async deleteEntry(t) {
    ...
    class d {
      constructor(t, { onSync: s, maxRetentionTime: i } = {}) {
        if (w.has(t)) throw new e.WorkboxError('duplicate-queue-name', { name: t });
        (w.add(t),
          (this.u = t),
      ...
      async g({ request: t, metadata: e, timestamp: s = Date.now() }, i) {
        const n = { requestData: (await o.fromRequest(t.clone())).toObject(), timestamp: s };
        (e && (n.metadata = e),
              ...
              if (t.tag === `${u}:${this.u}`) {
                const e = async () => {
                  ...
                  try {
                    ...
                    (!this.D || (e && !t.lastChance) || (await this.registerSync()),
                      (this.k = !1),
                      (this.D = !1));
                  }
                };
    ...
    const f = t => {
      const e = { request: new o(t.requestData).toRequest(), timestamp: t.timestamp };
