((this.workbox = this.workbox || {}),
  (this.workbox.core = (function (e) {
    ...
    const l = 'cacheDidUpdate',
      ...
      y = async ({ cacheName: e, request: t, event: n, matchOptions: s, plugins: r = [] }) => {
      ...
      m = async ({ request: e, response: t, event: n, plugins: s }) => {
        ...
        for (let t of s)
          if (
            h in t &&
            ((a = !0), !(r = await t[h].call(t, { request: e, response: r, event: n })))
          )
            break;
      ...
      q = async ({ request: e, mode: t, plugins: n }) => {
        ...
        let r = e;
        for (const e of s)
          'string' == typeof (r = await e[u].call(e, { mode: t, request: r })) &&
            (r = new Request(r));
        return r;
      },
      v = {
        put: async ({
          ...
          const f = await caches.open(e),
            w = p(a, l);
          let g = w.length > 0 ? await y({ cacheName: e, matchOptions: i, request: u }) : null;
          try {
            await f.put(u, h);
          } catch (e) {
    ...
    class x {
      constructor(e, t, { onupgradeneeded: n, onversionchange: s = this.t } = {}) {
      ...
      async getAllKeys(e, t, n) {
        return (await this.getAllMatching(e, { query: t, count: n, includeKeys: !0 })).map(
          ({ key: e }) => e,
        );
      }
      async getAllMatching(
        e,
        { index: t, query: n = null, direction: s = 'next', count: r, includeKeys: a } = {},
      ) {
    ...
    const D = {
      fetch: async ({ request: e, fetchOptions: t, event: s, plugins: r = [] }) => {
        ...
        try {
          for (let t of r) d in t && (e = await t[d].call(t, { request: e.clone(), event: s }));
        } catch (e) {
        ...
        try {
          ...
          for (const e of r)
            g in e && (n = await e[g].call(e, { event: s, request: c, response: n }));
          return n;
        } catch (e) {
          for (const t of a)
            await t[w].call(t, {
